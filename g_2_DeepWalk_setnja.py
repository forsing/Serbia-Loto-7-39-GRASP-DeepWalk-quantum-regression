...
# DeepWalk
# (različiti načini šetnje + Word2Vec/Doc2Vec)

# GRASP + kvantna regresija, deterministički, strukturno
# Varijanta: DeepWalk embedding

# Loto pipeline (embedding 39 čvorova → kvantna regresija → jedna kombinacija)
# pipeline (čvor → embedding → kvantna regresija)
# daje vektor po čvoru

"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data
DeepWalk = unbiased random walks + Word2Vec
"""
...
Graf iz M koji smo dobili iz draws, 
zatim težinske random šetnje 
(dužina 10, 80 šetnji po čvoru, seed=39), 
Word2Vec nad šetnjama (gensim, workers=1, seed=39), 
embedding za čvorove 1..39, normalizacija u [0, π]. 

Koristimo samo networkx i gensim.
...
WALK_LENGTH = 10 # dužina šetnje
NUM_WALKS = 80   # broj šetnji po čvoru
...
def compute_deepwalk_embeddings(M, k=EMBED_DIM):
    G = nx.Graph()
    for i in range(1, 40):
        G.add_node(i)
    for i in range(1, 40):
...
def main():
    ...
    emb = compute_deepwalk_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (DeepWalk + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    """
    Predikcija (DeepWalk + kvantna regresija, deterministički, strukturno):
    (8, 11, x, y, z, 34, 36)
    """
